...
function thunderBolt() {
  if(playerMove == 0 && userHP != 0) {
    ...
    if(miss == 1) {
      ...
      document.getElementById('message').innerHTML = " Pikachu used Thunder Bolt "; // attack
...
function spark() {
  ...
  if(miss == 1 ) {
    ...
    document.getElementById('message').innerHTML = " Pikachu used Spark ";
...
function electroBall() {
  ...
  if(miss == 1 ) {
    ...
    document.getElementById('message').innerHTML = " Pikachu used Electro Ball ";
    var critical = Math.floor((Math.random() * 10) + 1);
      if(critical == 4){
        for(var x = 0; x < 2; x++){
          opHP = opHP - 10;
